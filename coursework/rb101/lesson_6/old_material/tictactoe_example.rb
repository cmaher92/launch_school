...
class GameState
  ...
  # means other objects can read and write these attributes

  def initialize(current_player, board)
    self.current_player = current_player
    self.board = board # the board state
    self.moves = []    # the next available moves
  end
...
class GameTree
  def generate
    # sets the initial_game_state as a GameState object
    # generates possible moves by passing in the current GameState
  ...
  def generate_moves(game_state)
    ...
    # game_state.board at this point is an empty array
    # index represents position
    game_state.board.each_with_index do |player_at_position, position|
      unless player_at_position
      # if there isn't somebody at the position
        # generate a new board with a different location in memory
        next_board = game_state.board.dup
        # [nil, nil, nil, nil, ...]
        # ['X', nil, nil, nil, ...]
        next_board[position] = game_state.current_player

        # creates a new GameState with the next_player set to the opppsite
